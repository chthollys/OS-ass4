...
// Releases the mutex lock on the external context
static void io_double_unlock_ctx(struct io_ring_ctx *octx)
{
	mutex_unlock(&octx->uring_lock);
}

// Attempts to lock the external context with trylock or regular lock based on issue flags
...
// Releases the source file reference when a message ring operation completes or fails
...
// Determines if a message needs to be processed in the submitter's context
static inline bool io_msg_need_remote(struct io_ring_ctx *target_ctx)
{
	return target_ctx->task_complete;
}

// Handles completion of a message operation via task work queue
...
// Sets up a request for remote completion through the target context's task work queue
...
// Allocates a kiocb from the message cache or kmem_cache as a fallback
...
// Prepares and posts a data message to a remote target context
...
// Handles the details of sending a data message to another io_uring instance
...
// Processes a data message request to another io_uring instance
...
// Looks up and gets a reference to a file from the file table using the source fd
...
// Installs a file descriptor in the target ring and optionally posts a completion
...
// Handles completion of a file descriptor send operation via task work queue
...
// Prepares a request for remote file descriptor installation in the target context
...
// Processes a request to send a file descriptor to another io_uring instance
...
// Extracts and validates parameters from an SQE for a message ring operation
...
// Initializes a message ring request from an SQE
int io_msg_ring_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
	return __io_msg_ring_prep(io_kiocb_to_cmd(req, struct io_msg), sqe);
}

// Dispatches a message ring request based on the command type
...
// Allows sending a message to an io_uring instance without going through the normal request flow
