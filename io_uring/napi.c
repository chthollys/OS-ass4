...
// Searches for a NAPI entry in the hash list by its ID and returns it if found
...
// Converts busy loop time to kernel time by shifting and converting to nanoseconds
...
// Creates and adds a new NAPI entry with the given ID to the io_ring_ctx
...
// Removes a NAPI entry with the specified ID from the io_ring_ctx
...
// Removes timed-out entries from the NAPI list under lock protection
...
// Conditionally removes stale entries if the is_stale flag is true
...
// Determines if the busy loop has timed out by comparing current time with start time plus duration
...
// Checks various conditions to determine if the busy loop should terminate
 ...
 */
// Executes the busy loop for static tracking mode without reporting stale entries
...
// Executes the busy loop for dynamic tracking mode and identifies stale entries
static bool
...
// Routes busy loop execution to either static or dynamic tracking based on context settings
static inline bool
...
// Executes a blocking busy poll loop with appropriate end condition handling
 ...
 */
// Initializes NAPI data structures and default settings in the io_ring_ctx
 ...
 */
// Releases all NAPI resources by removing entries from both list and hash table
...
// Updates NAPI tracking mode and settings based on provided configuration
 ...
 */
// Registers NAPI with io-uring and handles various NAPI-related operations
 ...
 */
// Unregisters NAPI and resets all NAPI-related settings to inactive state
 ...
 */
// Executes the busy poll loop with appropriate timeout calculations
 ...
 */
// Performs busy polling specifically in submission queue polling mode
